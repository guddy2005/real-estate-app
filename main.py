...
def keep_alive():
    ...
    print(f"🚀 Keep-Alive: Starting self-ping thread for {url}")
    while True:
        try:
            # Ping every 14 minutes (Render sleeps after 15 mins)
            requests.get(url)
            print(f"✅ Keep-Alive: Self-ping successful at {datetime.now()}")
        except Exception as e:
            print(f"⚠️ Keep-Alive: Ping failed: {e}")
        time.sleep(840) # 14 minutes

# Start Keep-Alive thread if running in production (or if URL is provided)
